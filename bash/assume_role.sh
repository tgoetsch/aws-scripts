...
export AWS__SECRET_ACCESS_KEY=$(echo $new_session | jq -r '.Credentials.SecretAccessKey')
export AWS_SESSION_RTOKEN=$(echo $new_session | jq -r '.Credentials.SessionToken')
